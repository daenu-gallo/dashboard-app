...
const CustomerView = ({ domainMode = null }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data) {
        const grouped = {};
        data.forEach(img => {
          const idx = img.album_id || `idx_${img.album_index}`;
  ...
  // Password gate moved AFTER all hooks (see below)
  ...
  // Password gate (must be AFTER all hooks to comply with React rules)
